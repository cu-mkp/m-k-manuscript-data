...
|---|---|---|---|---|
| 0.1 ✅ | *(done, `60f96a39`)* Fix three malformed `<i>` tags in the comment CSV: *Bargeo…* (`<i>` for `</i>`, twice), *Cf., the modern Greek `<i>όφις<i/>`*, *Girolamo Mercuriale, `<i>Liber responsorum…`* (unclosed) | [R1] | `metadata/DCE_comment-tracking-Tracking.csv` | `grep -c '<i'` vs `'</i>'` balance per row |
...
### 1.1 — Balance inline tags in endnote text · [R1] · 🔴 · ✅ **DONE** (`60f96a39`)
* **Done:** three CSV rows corrected (`c_001r_14`, `c_013v_02`, `c_170v_27`) and `comment_text` now routed through `balance_inline_tags()`. Verified: pages ≥85% italic after p.540 went 205 → 0; `<i>` balanced in both HTML outputs.
* **Change (as shipped):** route `comment_text` through the existing `balance_inline_tags()` before interpolation in the endnotes assembly of `xml_to_html()`.
...
* **Also measure the back matter** (review A/V4 reports sparse, uneven index pages; the audit's void scan covered only the translation body, pp. 3–540). Re-measure *after* the R1 fix, which changed index pagination.
...
### 1.5 — Title-page CSS is fragile · [R13, from review A/V6] · 🟠
* **Change:** `.title-page { height: 8.5in; padding: 2em; border: … }` has no `box-sizing: border-box` (the property appears **zero** times in the stylesheet), so padding and border add to a fixed height inside a 9 in content area. The DCE branding header and footer added in `4d87b147` now share that box.
* **Fix:** `box-sizing: border-box; min-height: 8.5in;` plus a named page (`.title-page { page: title }`) rather than relying on the fixed height. Same change in the glossary and entry-metadata generators.
* **Verify:** rasterise p.1 of all four PDFs; confirm no overflow and the branding footer stays inside the border.
* **Effort:** 2 lines. Latent today, breaks on the next content change.
...
### 2.2b — Colour management and transparency · [R12, from review A/A5] · 🟠 for print-shop delivery
* **Finding (verified):** no `/OutputIntent` or ICC profile; **all 165 images are `/DeviceRGB`**; **153 carry an `/SMask`** soft mask. Older RIPs flatten transparency unpredictably, and print shops generally expect a colour-managed PDF/X or an explicit RGB policy.
* **Decision needed:** is the deposit/print copy deliberately digital-first sRGB, or does it need PDF/X-4 with an output intent? This is a print-shop conversation, not a code change.
* **Interacts with 4.1 and 2.3:** converting 165 RGB+SMask images to a print profile is a job to do **once**, after the figures are re-derived at print resolution.
...
* **Recommend:** defer until R3 (figure re-derivation) and R12 (colour policy) are resolved, or the images will be converted twice.
...
## Phase 5 — Automated build checks *(from review A's final section)*

Seed with the audit's own scripts — `qc/print-audit/step1_checks.py`, `step1_glyphs_images.py` — and run on every PDF build:

| Check | Fails on |
|---|---|
| Ghostscript full render to `nullpage` | any render error |
| Page-box + rotation consistency | any deviation |
| Font inventory | an un-embedded font, or any face outside the bundled/authored set (catches R4/R5 regressions) |
| Inline-tag balance in the emitted HTML | `<i>` opens ≠ closes (catches R1 regressions) |
| Sparse-page detector (bbox area, or trailing-void) | pages below a threshold outside intentional section openers |
| Effective image PPI | any placement below the agreed threshold (catches R3 regressions) |
| Internal-link resolution + named-destination count | unresolved destinations after post-processing |
| Metadata presence | missing `/Title`, `/Lang` (catches R9 regressions) |
| Missing-asset gate before WeasyPrint | any figure that fails to download |
...
| Print audit R12: no output intent; 153 soft masks, all-RGB images | 2.2b | pipeline + print shop |
| Print audit R13: title-page fixed height without box-sizing | 1.5 | pipeline |
| Print audit: automated build checks for the PDF pipeline | Phase 5 | pipeline |
